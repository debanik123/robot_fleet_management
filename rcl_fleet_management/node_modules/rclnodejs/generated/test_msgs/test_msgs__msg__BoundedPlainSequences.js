...
const BoundedPlainSequencesRefStruct = StructType({
...
const BoundedPlainSequencesRefArray = ArrayType(BoundedPlainSequencesRefStruct);
const BoundedPlainSequencesRefStructArray = StructType({
  data: BoundedPlainSequencesRefArray,
  size: ref.types.size_t,
  capacity: ref.types.size_t
});
// Define the wrapper class.
class BoundedPlainSequencesWrapper {
  constructor(other, willCheckConsistency = false) {
    ...
    if (typeof other === 'object' && other._refObject) {
      this._refObject = new BoundedPlainSequencesRefStruct(other._refObject.toObject());
  ...
  _initMembers() {
    this._refObject = new BoundedPlainSequencesRefStruct();
    this._wrapperFields.bool_values = BoolWrapper.createArray();
    this._wrapperFields.byte_values = ByteWrapper.createArray();
    this._wrapperFields.char_values = CharWrapper.createArray();
    this._wrapperFields.float32_values = Float32Wrapper.createArray();
  ...
  static createFromRefObject(refObject) {
    let self = new BoundedPlainSequencesWrapper();
    self.copyRefObject(refObject);
    return self;
  }
  static createArray() {
    return new BoundedPlainSequencesArrayWrapper;
  }
  static get ArrayType() {
    return BoundedPlainSequencesArrayWrapper;
  }
  static get refObjectArrayType() {
    return BoundedPlainSequencesRefStructArray
  }
  static get refObjectType() {
    return BoundedPlainSequencesRefStruct;
  }
  toRawROS() {
    this.freeze(true);
  ...
  freeze(own = false, checkConsistency = false) {
      if (checkConsistency && !this._alignment_checkIntialized) {
        throw new TypeError('Invalid argument: alignment_check in BoundedPlainSequences');
      }
    if (!own) {
  ...
  static destoryRawROS(msg) {
    BoundedPlainSequencesWrapper.freeStruct(msg.refObject);
  }
  static type() {
    return {pkgName: 'test_msgs', subFolder: 'msg', interfaceName: 'BoundedPlainSequences'};
  }
  static isPrimitive() {
    return false;
  }
  ...
  copyRefObject(refObject) {
    this._refObject = new BoundedPlainSequencesRefStruct(refObject.toObject());
    refObject.bool_values.data.length = refObject.bool_values.size;
    for (let index = 0; index < refObject.bool_values.size; index++) {
      this._bool_valuesArray[index] = refObject.bool_values.data[index].data;
    }
...
class BoundedPlainSequencesArrayWrapper {
  constructor(size = 0) {
    this._resize(size);
  }
  toRawROS() {
  ...
  fill(values) {
    ...
    values.forEach((value, index) => {
      if (value instanceof BoundedPlainSequencesWrapper) {
        this._wrappers[index].copy(value);
      } else {
        this._wrappers[index] = new BoundedPlainSequencesWrapper(value);
      }
    });
  }
  ...
  set size(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to BoundedPlainSequencesArrayWrapper.size setter');
      return;
    }
  ...
  set capacity(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to BoundedPlainSequencesArrayWrapper.capacity setter');
    }
    return this._resize(value);
  }
  get refObject() {
  ...
  _resize(size) {
    ...
    this._refArray = new BoundedPlainSequencesRefArray(size);
    this._refObject = new BoundedPlainSequencesRefStructArray();
    this._refObject.size = size;
    this._refObject.capacity = size;
    this._wrappers = new Array();
    for (let i = 0; i < size; i++) {
      this._wrappers.push(new BoundedPlainSequencesWrapper());
    }
  }
  ...
  copy(other) {
    if (! (other instanceof BoundedPlainSequencesArrayWrapper)) {
      throw new TypeError('Invalid argument: should provide "BoundedPlainSequencesArrayWrapper".');
    }
  ...
  static freeArray(refObject) {
    ...
    for (let index = 0; index < refObject.size; index++) {
      BoundedPlainSequencesWrapper.freeStruct(refObjectArray[index]);
    }
  }
...
module.exports = BoundedPlainSequencesWrapper;
/*
